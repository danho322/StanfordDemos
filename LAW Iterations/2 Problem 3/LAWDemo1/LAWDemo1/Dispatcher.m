...
-(int)getTemplateIDForDocument:(CXMLDocument*)doc
{
    ...
    {    
        ...
        else if ([templateString isEqualToString:@"experiment"])
            return kTemplateIDExperiment;
    }   
...
-(TemplateWindow*)getTemplateWindowForDocument:(CXMLDocument*)doc
{
    ...
    {
        return [[[TemplateID1 alloc] initWithDictionary:dict] autorelease];
        
    }
    else if (templateID == kTemplateID32)
    {
        return [[[Template32 alloc] initWithDictionary:dict] autorelease];
    }
    else if (templateID == kTemplateIDExperiment)
    {
        return [[[TemplateExperiment alloc] initWithDictionary:dict] autorelease];
    }
    return nil;
}
...
-(NSMutableDictionary*)getDictionaryForTemplateID:(int)templateID forDocument:(CXMLDocument*)doc
{
    ...
    else if (templateID == kTemplateIDExperiment)
    {
        ...
        node = [self getNodeWithString:@"sentence" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        //nodes = [NSArray arrayWithObject:node];
        NSString *sentence = node.stringValue;
        
        node = [self getNodeWithString:@"question" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSMutableArray *questionArray = [[[NSMutableArray alloc] init] autorelease];
        [self searchForAttributes:@"text" inNodes:nodes putIntoArray:questionArray];
        
        node = [self getNodeWithString:@"wordlistnumber" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSString *wordlistnumber = [self searchForAttribute:@"value" inNodes:nodes];
        
        node = [self getNodeWithString:@"filename" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSString *filename = [self searchForAttribute:@"value" inNodes:nodes];
        
        node = [self getNodeWithString:@"parser" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSString *parser = [self searchForAttribute:@"value" inNodes:nodes];
        
        node = [self getNodeWithString:@"parserexercise" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSString *parserexercise = [self searchForAttribute:@"value" inNodes:nodes];
        
        node = [self getNodeWithString:@"defaultErrorMessage" forAttribute:@"name" withXPath:@"//property" fromDoc:doc];
        nodes = [NSArray arrayWithObject:node];
        NSString *defaultErrorMessage = [self searchForAttribute:@"value" inNodes:nodes];
        
        
        dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:fileID, @"fileID",
                                                                answer, @"answer",
                                                                sentence, @"sentence",
                                                                questionArray, @"question",
                                                                wordlistnumber, @"wordlistnumber",
                                                                filename, @"filename",
                                                                parser, @"parser",
                                                                parserexercise, @"parserexercise",
                                                                defaultErrorMessage, @"defaultErrorMessage", nil];
    }
